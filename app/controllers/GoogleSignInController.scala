...
class GoogleSignInController @Inject()(
                                        ...
                                        googleTokenVerifier: GoogleOauthService) extends Controller {
  ...
  def authenticate(): Action[JsValue] = Action.async(BodyParsers.parse.json) { implicit request =>
    for {
      ...
      gUser <- googleTokenVerifier.verifyToken(token) ?| Unauthorized(Json.obj("message" -> "User token is invalid!"))
