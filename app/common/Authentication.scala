...
class Authentication @Inject()(userService: UserService, oauth: OauthService, config:Config) extends LazyLogging {
  ...
  class UserAction(accessLevel: AccessLevel) extends
    ...
    private def authenticateApi[A](request: Request[A], block: (UserRequest[A]) => Future[Result]): Future[Result] = {
      block(new UserRequest("", User(None, "api", "call", "api@call.com", AccessLevel.Admin, Status.Active), request))
    }
